...
@torch.compile(dynamic=False, fullgraph=True)
def hybrid_sed_step_fused(
    stacked_grads: Tensor,          # (K, R, C)
    stacked_params: Tensor,         # (K, R, C)
    momentum_buffer: Tensor,        # (K, R, C)
    second_momentum_buffer: Tensor, # (K, R, 1) or (K, 1, C)
    momentum_t: Tensor,
    lr_t: Tensor,
    wd_t: Tensor,
    beta2_t: Tensor,
    sed_val_t: Tensor,              # 0-D scalar: SED parameter
    use_muon: bool,             # 0-D boolean: True for Muon, False for SED
    ns_steps: int,
    red_dim: int,
) -> None:
    """
    Fused Hybrid Step:
    1. Momentum
    2. Branch: Muon (Polar Express) OR SED (Spectral-Elastic)
    3. Variance Reduction (NorMuon style, applied to both for stability)
    4. Cautious Update
    """
    ...
    # --- 2. Orthogonalization / SED Transform ---
    # Convert to bfloat16 for matrix multiplications
    X = g.bfloat16()
    
    # Pre-normalization (critical for stability)
    X = X / (X.norm(dim=(-2, -1), keepdim=True) + 1e-7)

    if use_muon:
        # === Muon Phase: Polar Express ===
        # Standard Muon scaling for Newton-Schulz
        X = X * 1.02 # Small boost for Newton-Schulz
        ...
        # === SED Phase: Spectral-Elastic Descent ===
        # print("***** SED *****")
        # Logic adapted from SED paper / provided code
        sed = sed_val_t.to(X.dtype)
        a = sed
        b = 2.5 - 2 * sed
        c = sed - 1.5
        
        # Determine transposes to optimize for smaller Gram matrix
        transposed = False
        ...
        for _ in range(ns_steps):
            A = X @ X.mT
            B = A @ X
            # X_new = a*X + b*B + c*(A@B) -> factorized: a*X + b*(XX^T)X + c*((XX^T)^2)X
            # Note: B is already (XX^T)X. A@B is (XX^T)^2 X.
            X = a * X + b * B + c * (A @ B)
    ...
    # Apply to both Muon and SED phases to ensure update magnitude consistency
    beta2 = beta2_t.to(g.dtype)
    v_mean = g.float().square().mean(dim=red_dim, keepdim=True)
    red_dim_size = g.size(red_dim)
    v_norm_sq = v_mean.sum(dim=(-2, -1), keepdim=True) * red_dim_size
...
# ==========================================
# 3. HybridSED Optimizer (Single GPU)
# ==========================================

class HybridSED(torch.optim.Optimizer):
    def __init__(self, param_groups: list[dict]):
        super().__init__(param_groups, defaults={})
        # 0-D CPU tensors
        # AdamW tensors
        ...
        self._muon_beta2_t = torch.tensor(0.0, dtype=torch.float32, device="cpu")
        # New Hybrid tensors
        self._sed_val_t = torch.tensor(0.0, dtype=torch.float32, device="cpu")
        # self._use_muon_t = torch.tensor(True, dtype=torch.bool, device="cpu") # Boolean tensor
    ...
    def _step_hybrid(self, group: dict) -> None:
        """Hybrid Muon/SED step"""
        params: list[Tensor] = group['params']
        if not params: return

        # Get State (anchor to first param)
        p = params[0]
        state = self.state[p]
        
        # --- State Init & Step Count ---
        # Note: We track step at group level via the first param
        ...
        # Hybrid Logic: Determine Phase
        total_steps = group.get('total_steps', 100000) # Default backup
        muon_percent = group.get('muon_percent', 0.5)
        switch_step = total_steps * muon_percent
        use_muon = current_step <= switch_step
        ...
        # New: Fill SED specific tensors
        self._sed_val_t.fill_(group.get('sed_val', 0.0))
        # self._use_muon_t.fill_(use_muon)

        # Call Fused Kernel
        hybrid_sed_step_fused(
            stacked_grads,
            stacked_params,
            momentum_buffer,
            second_momentum_buffer,
    ...
    def step(self):
        for group in self.param_groups:
            if group['kind'] == 'adamw':
                self._step_adamw(group)
            elif group['kind'] == 'muon': # Keeps name 'muon' for config compatibility
                self._step_hybrid(group)
            else:
                raise ValueError(f"Unknown optimizer kind: {group['kind']}")


# ==========================================
# 4. Distributed HybridSED (DistHybridSED)
# ==========================================

class DistHybridSED(torch.optim.Optimizer):
    """
    Distributed version of HybridSED. 
    Handles async communication and state sharding.
    """
    def __init__(self, param_groups: list[dict]):
        super().__init__(param_groups, defaults={})
        # 0-D Tensors
        ...
        # New Tensors
        self._sed_val_t = torch.tensor(0.0, dtype=torch.float32, device="cpu")
        # self._use_muon_t = torch.tensor(True, dtype=torch.bool, device="cpu")
    ...
    def _reduce_hybrid(self, group: dict, world_size: int) -> dict:
        params = group['params']
        chunk_size = (len(params) + world_size - 1) // world_size
        padded_num_params = chunk_size * world_size
        p = params[0]
    ...
    def _compute_adamw(self, group: dict, info: dict, gather_list: list, rank: int, world_size: int) -> None:
        ...
        for p in group['params']:
            ...
            # --- State Init & Step Count ---
            if not state:
                # State init for distributed sharded state
                if pinfo['is_small']:
                    state['exp_avg'] = torch.zeros_like(p)
                    state['exp_avg_sq'] = torch.zeros_like(p)
                else:
    ...
    def _compute_hybrid(self, group: dict, info: dict, gather_list: list, rank: int) -> None:
        """Modified to include switching logic using state['step']"""
        info['future'].wait()
        params = group['params']
        chunk_size = info['chunk_size']
        grad_chunk = info['grad_chunk']
        ...
        # Hybrid Logic
        total_steps = group.get('total_steps', 100000)
        muon_percent = group.get('muon_percent', 0.5)
        switch_step = total_steps * muon_percent
        use_muon = current_step <= switch_step
        ...
        if num_owned > 0:
            ...
            hybrid_sed_step_fused(
                ...
                state["momentum_buffer"][:num_owned], state["second_momentum_buffer"][:num_owned],
                self._muon_momentum_t, self._muon_lr_t, self._muon_wd_t, self._muon_beta2_t,
                self._sed_val_t, use_muon, # Pass new args
                group["ns_steps"], red_dim,
            )
            updated_params[:num_owned].copy_(stacked_owned)
    ...
    def step(self):
        ...
        for group in self.param_groups:
            if group['kind'] == 'adamw':
                ...
                reduce_infos.append(self._reduce_hybrid(group, world_size))
            else:
                raise ValueError(f"Unknown optimizer kind: {group['kind']}")
        ...
        for group, info in zip(self.param_groups, reduce_infos):
            if group['kind'] == 'adamw':
                ...
                self._compute_hybrid(group, info, gather_list, rank)
            else:
                raise ValueError(f"Unknown optimizer kind: {group['kind']}")
